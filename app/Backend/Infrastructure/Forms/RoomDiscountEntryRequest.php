...
class RoomDiscountEntryRequest extends Request
{
    /**
    ...
    {
        return [
            'name'              => 'required|unique:room_discount,name,NULL,id,hotel_id,'.Input::get('hotel_id').',h_room_category_id,'.Input::get('h_room_category_id').',from_date,'.Carbon::parse(Input::get('from_date'))->format('Y-m-d').',to_date,'.Carbon::parse(Input::get('to_date'))->format('Y-m-d').',deleted_at,NULL',
            'hotel_id'          => 'required',
